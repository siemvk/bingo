...
function setFase(newFase = "preparing") {
  if (newFase === "playingBingo" && fase === "preparing") {
    ...
    players.forEach((p) => {
      if (!playerCards[p])
        playerCards[p] = generateBingoCard();
      if (!playerMarked[p])
        playerMarked[p] = [];
    });
  }
...
ws.addEventListener("message", (ev) => {
  ...
  if (msg.type == "playerUpdate") {
    if (msg.update == 1 /* connect */) {
      ...
      if (!playerCards[msg.player])
        playerCards[msg.player] = generateBingoCard();
      if (!playerMarked[msg.player])
        playerMarked[msg.player] = [];
  ...
  if (msg.type == "UI-msg") {
    ...
    if (msg.elementInteractedWith.id == "markBtn") {
      ...
      if (markInput && markInput.value) {
        const num = parseInt(markInput.value, 10);
...
function validateBingo(player) {
  const card = playerCards[player];
  if (!card)
    return false;
  const marks = playerMarked[player] || [];
  const isValid = (cell) => cell === "FREE" || marks.includes(cell) && drawnNumbers.includes(cell);
  if (bingoMode === "full") {
    return card.every((row) => row.every(isValid));
  } else {
    for (let r = 0;r < 5; r++)
      if (card[r].every(isValid))
        return true;
    for (let c = 0;c < 5; c++) {
      let colMarked = true;
      for (let r = 0;r < 5; r++) {
    ...
    case "playingBingo":
      ...
      for (const row of card) {
        ...
        for (const cell of row) {
          const isMarked = cell === "FREE" || marks.includes(cell);
  ...
  if (msg.type == "UI-msg") {
    if (msg.elementInteractedWith.id == "setFase") {
      ...
      if (newModeEl?.type == "field" && newModeEl.value) {
        bingoMode = newModeEl.value;
      }
      if (newFaseEl?.type == "field" && newFaseEl.value) {
        setFase(newFaseEl.value);
      }
...
function renderQR(elId, pinCode) {
  ...
  import_qrcode.default.toCanvas(canvas, textToEncode, {
    width: 200,
    margin: 2,
